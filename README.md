...
As sometimes happens to all of us, we are presented with a solution but don't yet have a problem to solve! In this case the solution was that I got the chance to borrow a programmable robot arm for a few weeks, but as the robot was delivered much earlier than expected, I had not thought about a use case for it. So I needed to decide about what objects to pick and place using the suction cup, as well as what software to use for controlling the robot. As physical objects I decided to use playing cards as they are uniform and lightweight. For the controlling software I had initially thought about only using Python, but I quickly moved on to explore how to also use TinyML (Tiny Machine Learning) for a more rewarding experience.

This project is part one of two, showing how to classify poker cards into three categories, by using Edge Impulse as well as the Silicon Labs xG24 DevKit and an Arducam camera. Part two continues with using the same hardware setup for controlling the robot arm to sort cards, but also showcases how to easily adapt it to sorting waste.
